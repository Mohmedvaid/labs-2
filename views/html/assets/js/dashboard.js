$(document).ready(function () {
  ...
  function handleSignUpErrors(errors) {
    ...
    if (errors.message) {
      $(`#error`).append(
        `<div id="main-err" class="text-danger">${errors.message}<div>`
      );
      return;
    }
    let errorEl = $(`#error`);
  ...
  function initLocations() {
    axios.get("/api/location").then((res) => {
      ...
      if (locations.length === 0)
        return checkBoxDiv.append(`<p>No locations added yet</p>`);
  ...
  function initAllUsers() {
    axios.get("/api/users").then(function (response) {
      ...
      userSelect.empty();
      if (users.length === 0)
        return userSelect.append(`<option>No users added yet</option>`);
  ...
  $(`#getLeadsByUsers`).click(function (e) {
    ...
    if (date.toLowerCase() === "select a date range")
    ...
    axios
      .get(
        `/api/customers/byUser/${userID}?fromDate=${fromDateUTC}&toDate=${toDateUTC}`
      )
      .then((customerData) => {
  ...
  function initDuplicateCustomers(customers) {
    ...
    if (duplicateCount === 0) {
      table.append(
        `<p id="noDuplicateCustomer">No duplicate customers found</p>`
      );
    }
